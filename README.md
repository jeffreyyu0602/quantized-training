...
Fine-tuning the Transformer models for sequence classification on the GLUE benchmark and question answering on the SQuAD v1.1. GLUE is made up of a total of 9 different tasks. In our paper, we conduct evaluations on three benchmarks: SST-2, MRPC, and QNLI. All commands required to reproduce the results presented in Table 4 are provided in the script named `asplos_training.sh`. The experiments are divided into multiple groups, with each group consisting of four experiments. Specifically, the first set of experiments involves four distinct tasks with different data types and configurations: BF16, Posit8, Posit8 with approximation, and FP8 on the MobileBERT-tiny model for the QNLI task. This corresponds to the results shown in the first major row (MobileBERT-tiny) and the first column (QNLI) of Table 4. The structure for the remaining groups of experiments follows this same organizational pattern.
