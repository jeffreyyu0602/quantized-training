...
class WhisperAttention(nn.Module):
    ...
    def forward(
        ...
        # get query proj
        query_states = self.q_scaling(self.q_proj(hidden_states), self.scaling)
        # get key, value proj
        ...
        attn_weights = self.qk_matmul(query_states, key_states.transpose(1, 2))
        ...
        attn_output = self.av_matmul(attn_probs, value_states)
...
class WhisperEncoderLayer(nn.Module):
    def __init__(self, config: WhisperConfig):
        ...
        self.self_attn_residual = AddFunctional()
        self.ffn_residual = AddFunctional()
    ...
    def forward(
        ...
        hidden_states = self.self_attn_residual(residual, hidden_states)
        ...
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        hidden_states = self.ffn_residual(residual, hidden_states)
...
class WhisperDecoderLayer(nn.Module):
    def __init__(self, config: WhisperConfig):
        ...
        self.final_layer_norm = nn.LayerNorm(self.embed_dim)
        self.self_attn_residual = AddFunctional()
        self.encoder_attn_residual = AddFunctional()
        self.ffn_residual = AddFunctional()
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            hidden_states = self.encoder_attn_residual(residual, hidden_states)
