...
class GPT2Attention(nn.Module):
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        attn_weights = self.qk_matmul(query, key.transpose(-1, -2))

        if self.scale_attn_weights:
            # attn_weights = attn_weights / torch.full(
            #     [], value.size(-1) ** 0.5, dtype=attn_weights.dtype, device=attn_weights.device
            # )
            attn_weights = self.attn_scaling(attn_weights, 1.0 / value.size(-1) ** 0.5)
        ...
        attn_output = self.av_matmul(attn_weights, value)
...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.attn_residual = AddFunctional()
        self.crossattention_residual = AddFunctional()
        self.mlp_residual = AddFunctional()
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            # residual connection
            hidden_states = self.crossattention_residual(residual, attn_output)
        ...
        # residual connection
        hidden_states = self.mlp_residual(residual, feed_forward_hidden_states)
